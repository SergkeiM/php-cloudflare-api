...
params:
  ...
  - name: "values"
    type: "array"
    required: true
    description: "`flows` is required and replaces the whole configuration; an empty list clears it. `etag` from `get()` makes the write conditional on nothing having changed since. `version` is the request's schema version, defaulting to the `2` Cloudflare accepts."
---
::
