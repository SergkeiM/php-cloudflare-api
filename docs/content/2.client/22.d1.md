...
params:
  ...
  - name: "values"
    type: "array"
    required: true
    description: "`name` is required and must match `^[a-z0-9][a-z0-9-_]*$`. `primary_location_hint` (`wnam`, `enam`, `weur`, `eeur`, `apac` or `oc`) places the primary, defaulting to near the caller. `read_replication` and `jurisdiction` are optional."
---
::
