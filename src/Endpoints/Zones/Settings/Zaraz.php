...
namespace Cloudflare\Endpoints\Zones\Settings;
...
class Zaraz extends AbstractEndpoint
{
     ...
     *
     * ```php
     * $config = $client->zones()->settings()->zaraz()->getConfig('ZONE_ID')->json('result');
     * $config['debugKey'] = 'new-debug-key';
     *
     * $client->zones()->settings()->zaraz()->updateConfig('ZONE_ID', $config);
     * ```
     *
     ...
     *
     * @link https://developers.cloudflare.com/api/resources/zaraz/subresources/workflow/methods/update/
     *
     * @param string $zoneId Zone Identifier.
